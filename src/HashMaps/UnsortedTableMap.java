...
public class UnsortedTableMap<K, V> extends AbstractMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        if(i == -1)
        {
            table.add(new MapEntry<>(key, value));  //create new entry
            return null;
        }

        else
        {
            return table.get(i).setValue(value);    //replaced value returned
        }
    }
    ...
    public V remove(K key) {
        ...
        if(i != size - 1)
        {
            table.set(i, table.get(size - 1));  //place last entry in place of removed one
        }

        table.remove(size - 1);   //remove last table entry
        return result;
    }
